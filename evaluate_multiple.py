...
def build_parser():
    ...
    parser.add_argument('--loop-num', type=int,
                        dest='loop_num', default=1)
...
def main():
    ...
    if os.path.isfile(opts.in_path):
        if os.path.isdir(opts.out_path):
                ...
                out_path, fextension = os.path.splitext(out_path)
                in_path = opts.in_path
                
                for i in range(opts.loop_num):
                    ffwd_to_img(in_path, out_path + str(i) + fextension, opts.checkpoint_dir,device=opts.device)
                    in_path = out_path + str(i) + fextension

        else:
            print('out_path is not dir.')

    else:
        print('in_path is not a file.')
